...
class TestChainRegistryRun:
    ...
    def test_run_multi_step_chain(self):
        ...
        # Register chain with custom coords
        ...
        async def _run():
            result = await cat.chain_registry.run(
                cat, "test_reasoning", prompt="hello",
            )
            # generate returns {"text": "reply_to:hello"}
            # say receives text="reply_to:hello"
            assert result == {"spoken": "reply_to:hello"}
            assert called["generate"] == "hello"
            assert called["say"] == "reply_to:hello"
