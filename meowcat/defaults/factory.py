...
Unprovided organs automatically use Default* / InMemory* default implementations.

v2.0: Default + Renovated merged — all organs have real behavior by default.
"""
...
def create_cat(
    *,
    ...
    All organs use merged (Default+Renovated) defaults — safety regex,
    ...
    cat.hippocampus = hippocampus or DefaultHippocampus()
    _maybe_register_hippo_lifecycle(cat)
    # type: ignore[attr-defined]
    cat.thalamus = thalamus or DefaultThalamus(keyword=keyword)
    # type: ignore[attr-defined]
    cat.amygdala = amygdala or DefaultAmygdala(
        keyword=keyword,
        dangerous_tools=dangerous_tools,
        dangerous_paths=dangerous_paths,
    )
    # type: ignore[attr-defined]
    cat.frontal = frontal or DefaultFrontal(
        keyword=keyword,
    ...
    # type: ignore[attr-defined]
    cat.hypothalamus = hypothalamus or DefaultHypothalamus()
    # type: ignore[attr-defined]
    cat.cerebellum = cerebrum if cerebellum is _UNSET else cerebellum
    cat.cerebrum = cerebrum  # type: ignore[attr-defined]
    # type: ignore[attr-defined]
    cat.cortex = cortex or DefaultCortex()
    cat.brainstem = brainstem  # type: ignore[attr-defined]

    # -- Senses ----------------------------------------------------------
    # type: ignore[attr-defined]
    cat.ears = ears or DefaultEars(keyword=keyword)
    # type: ignore[attr-defined]
    cat.eyes = eyes or DefaultEyes()
    # type: ignore[attr-defined]
    cat.whiskers = whiskers or DefaultWhiskers()
    # type: ignore[attr-defined]
    cat.paws = paws or DefaultPaws()

    # -- Outputs ---------------------------------------------------------
    # type: ignore[attr-defined]
    cat.mouth = mouth or DefaultMouth()
    # type: ignore[attr-defined]
    cat.purr = purr or DefaultPurr()
    # type: ignore[attr-defined]
    ...
    # type: ignore[attr-defined]
    cat.correction_growth = correction_growth or DefaultCorrectionGrowth()
    # type: ignore[attr-defined]
    cat.crystallizer = crystallizer or DefaultCrystallizer(
        crystallize_threshold=crystallize_threshold,
        hotspot_threshold=hotspot_threshold,
    )
    # type: ignore[attr-defined]
    cat.role_emergence = role_emergence or DefaultRoleEmergence()
