...
>
> ⚠️ **v2.0 破坏性变更** — 从 v1.x 升级前先读 **[MIGRATION_v2.md](MIGRATION_v2.md)** (中文) · **[MIGRATION_v2_EN.md](MIGRATION_v2_EN.md)** (English)
...
```
Gateway → Colony → Cat
                      ├── perceive() / do_task()   ← 打工循环
                      └── ReflectionLoop           ← 成长循环
```

meowcat 呈现 **两层模型**：

- **打工循环** — `perceive()` / `do_task()` — 猫的意识活动。Hear → Route → Reason → Speak。工具通过 brain↔paws 多轮循环执行。
- **成长循环** — `ReflectionLoop` — 自我进化。草稿 → 蒸馏 → 融合到看板和猫舍。

完整 20 器官蓝图参见 [AGENTS.md](AGENTS.md) 和 [CATALOG.md](CATALOG.md)。
