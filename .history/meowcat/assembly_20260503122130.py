...
# v1.0.14: 生命周期钩子类型 — 接受 CatBase 实例的异步可调用对象
...
class CatBase:
    ...
    def use_middleware(self, mw: Any) -> None:
        ...
        Args:
            mw: 实现 :class:`~meowcat.nervous.SignalMiddleware` Protocol 的中间件实例
    ...
    def register_workflow(self, wf: dict[str, Any]) -> None:
        ...
        应用层创建 WorkflowShape 后调用此方法注册，框架自动在
        shutdown 时存档 checkpoint。

        Args:
            wf: WorkflowShape 的 dict 形式，必须含 ``entity_id`` 键
        """
    ...
    async def _resume_workflows(self) -> None:
        """扫描 Hippocampus 中未完成的 Workflow 并加载到 _active_workflows。

        静默失败：Hippocampus 未挂载或查询异常不影响启动。
        """
    ...
    async def _checkpoint_workflows(self) -> None:
        """遍历所有活跃 Workflow，写 checkpoint 到 Hippocampus。

        静默失败：Hippocampus 未挂载或写入异常不影响关闭。
        """
    ...
    def on_start(self, hook: CatHook) -> None:
        ...
        Args:
            hook: 接受 CatBase 实例的异步可调用对象，在 ``start()`` 发射
                  lifecycle.start 事件后调用。
    ...
    def on_shutdown(self, hook: CatHook) -> None:
        ...
        Args:
            hook: 接受 CatBase 实例的异步可调用对象，在 ``shutdown()``
                  发射 lifecycle.shutdown 事件前逆序调用。
    ...
    async def start(self) -> None:
        """启动猫。扫描未完成 Workflow → 发射 lifecycle.start → 依次调用 on_start hooks。

        子类可重写，**务必调用 ``await super().start()``**。
        """
        # v1.0.15: 扫描 Hippocampus 中未完成的 Workflow 并加载
    ...
    async def shutdown(self) -> None:
        """关闭猫。存档活跃 Workflow → 逆序调用 on_shutdown hooks → 发射 lifecycle.shutdown。

        子类可重写，**务必调用 ``await super().shutdown()``**。
        """
        # v1.0.15: 存档所有活跃 Workflow 到 Hippocampus
