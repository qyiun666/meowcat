"""meowcat 独立测试: v0.5.28b Loop 闭环 + LoopRegistry + 自环 Path。

验证:
- Loop dataclass 不可变性 + 默认值
- BUILTIN_LOOPS 5 条内置闭环
- LoopRegistry register / get / list_all
- LoopRegistry.run() 触发事件 → 跑 chain → 退出事件
- 自环 Path (from == to) 直接调本地方法
- CatBase 自动注册 5 条内置闭环
- cat.run_loop() 门面方法
"""
...
class TestBuiltinLoops:
    ...
    def test_diagnostic_loop_no_trigger(self):
        ...
        assert lp.trigger is None
        assert lp.chain.path_names == ()  # 空链路
...
class TestLoopRegistryRun:
    ...
    def _setup_cat(self):
        """创建带 fake 器官 + events 追踪的 CatBase。"""
        cat = CatBase("test")
        TH = ("brain", "_thalamus")
...
class TestSelfLoopPath:
    ...
    def test_self_loop_path_run(self):
        """自环 path 不经过 wiring signal，直接调用本地方法。"""
        cat = CatBase("test")
        TH = ("brain", "_thalamus")
...
class TestCatBaseLoopIntegration:
    """CatBase 自动注册 5 条内置闭环 + run_loop() 门面。"""
    ...
    def test_conversation_loop_chain_registered(self):
        """内置闭环的内联 Chain 已在 chain_registry 中。"""
        cat = CatBase("test")
    ...
    def test_from_meowcat_import_loops(self):
        """外部可用 from meowcat import Loop/LoopRegistry/BUILTIN_LOOPS。"""
...
class TestNewPathsForLoop:
    ...
    def test_total_path_count(self):
        """BUILTIN_PATHS 当前 23 条（v1.0.11 新增 synthesize）。"""
        cat = CatBase("test")
