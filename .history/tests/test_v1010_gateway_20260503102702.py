...
验证:
    ...
    2. Protocols — isinstance 检查 / AdapterProtocol 实现
...
class TestProtocols:
    """GatewayProtocol / AdapterProtocol isinstance 检查。"""
    ...
    def test_cli_adapter_is_adapter_protocol(self) -> None:
        """CliAdapter 满足 AdapterProtocol。"""
        assert isinstance(CliAdapter(), AdapterProtocol)

    def test_http_adapter_is_adapter_protocol(self) -> None:
        """HttpAdapter 满足 AdapterProtocol。"""
        assert isinstance(HttpAdapter(), AdapterProtocol)

    def test_ws_adapter_is_adapter_protocol(self) -> None:
        """WsAdapter 满足 AdapterProtocol。"""
        assert isinstance(WsAdapter(), AdapterProtocol)
...
class TestHttpAdapter:
    ...
    async def test_post_missing_message(self) -> None:
        ...
        server_task = asyncio.create_task(adapter.serve(on_message, on_stream))
        await asyncio.sleep(0.1)
        actual_port = adapter._server.sockets[0].getsockname()[1] if adapter._server else adapter.port
...
class TestCliAdapter:
    ...
    async def test_cli_input_output(self) -> None:
        ...
        with patch.object(sys, "stdin", fake_stdin):
            # serve 在读到空行后会退出
            serve_task = asyncio.create_task(adapter.serve(on_message, on_stream))
...
class TestWebhookAdapter:
    ...
    async def test_webhook_post_callback(self) -> None:
        ...
        try:
            reader, writer = await asyncio.open_connection("127.0.0.1", actual_port)
            body = json.dumps({"message": "webhook hi", "user_id": "u42"}).encode()
            request = (
    ...
    async def test_webhook_signature_rejected(self) -> None:
        """签名验证失败返回 403。"""
        adapter = WebhookAdapter(port=0)
        adapter.verify_signature = lambda headers, body: False  # type: ignore[method-assign]
...
class TestIpcAdapter:
    ...
    async def test_ipc_multiple_messages(self) -> None:
        ...
        try:
            reader, writer = await asyncio.open_unix_connection(socket_path)
            for i in range(3):
                writer.write(json.dumps({"message": f"msg{i}"}).encode() + b"\n")
                await writer.drain()
                response = await asyncio.wait_for(reader.readline(), timeout=5)
