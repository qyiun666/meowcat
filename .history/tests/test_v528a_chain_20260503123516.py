"""meowcat 独立测试: v0.5.28a Chain 链路 + ChainRegistry。

验证:
- Chain dataclass 不可变性 + 默认值
- ChainRegistry register / get / list_all
- ChainRegistry.run() 按序执行 path，结果传递
- 空链路、单步链路、多步链路
- 不存在的 chain → KeyError
- CatBase 自动注册 5 条内置链路
"""
...
class TestChainRegistryRun:
    ...
    def _setup_cat(self):
        ...
        # 注册 paths: 用自定义坐标（未映射坐标避免 Protocol 契约校验）
    ...
    def test_run_multi_step_chain(self):
        """多步链路：手动注册 full_reasoning = ('deep_reason', 'say')。

        前一步返回值作为下一步 kwargs：generate 返回 {"text":...}→
        say 收到 text=...（key 名需与下游参数名对齐）。
        """
        cat, called = self._setup_cat()
